<template>
  <view class="child-content">
    ...
    <!-- 后续处理意见 请把ajax写在该组件里 -->
    <view v-if="hxclyjShow">
      <xcdd-hxclyj :hxcly="hxclyj" :contentId='contentId' :zgxsId="zgxsid" @close="closeHxclyj" @confirm="confirmHxclyj" 
...
<script>
  ...
  export default {
    ...
    onShow() {
      // 从评估页面过来是返回页面栈的方式，所以要监听页面显示，一次重新获取评估分值
      if(this.mxid){
        this.getPgxq()
      }
    },
...
// 				}
      ...
      // 保存督导
      saveXcdd (sfXq) {
        if (!this.xx.value || !this.ywsj || !this.getDdjs()) {
          this.$kwz.alert('学校、时间、督导纪实为必填项')
          return
        }
        let xcddData = {
