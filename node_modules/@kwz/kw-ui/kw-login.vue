...
<template>
  <view class="kw-login" >
    ...
    <form class="kw-login-form">
      ...
      <view class="kw-login-input_bg" >
        <button v-if="!loadingLogin" @tap="login()">登录</button>
        <button v-else :loading="true">登录</button>
      </view>
...
//                 }, this)
            ...
            } else if(data.statusCode == '500'){ // 未输入验证码
              this.vcodeShow = true
              this.toggerVcode()
              this.loadingLogin = false
            } else if(data.statusCode == '102' || (data.statusCode == '304' && !this.vcodeShow) ){ // 102图片验证码失效 304账号密码错误3||2次，出现验证码
              this.toggerVcode()
              this.loadingLogin = false
            } else if(data.statusCode == '315'){ // 账号密码错误
              this.toggerVcode()
              this.loadingLogin = false
            }
          },
